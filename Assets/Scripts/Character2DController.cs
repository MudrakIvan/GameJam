...
public class Character2DController : MonoBehaviour
{
    ...
    /// </summary>
    RaycastHit2D _unstuckRaycastHit;
    ...
    void moveHorizontally( ref Vector3 deltaMovement )
    {
        ...
        for( var i = 0; i < totalHorizontalRays; i++ )
        {
            ...
            if (i == 0 && collisionState.wasGroundedLastFrame)
            {
                ...
                _unstuckRaycastHit = Physics2D.Raycast( ray, -rayDirection, rayDistance, platformMask );
            }
            else
            {
                _raycastHit = Physics2D.Raycast( ray, rayDirection, rayDistance, platformMask & ~oneWayPlatformMask );
                _unstuckRaycastHit = Physics2D.Raycast( ray, -rayDirection, rayDistance, platformMask & ~oneWayPlatformMask );
            }

            if (_unstuckRaycastHit)
            {
                if (isGoingRight)
                { deltaMovement.x = _unstuckRaycastHit.point.x - boxCollider.bounds.max.x; }
                else
                { deltaMovement.x = _unstuckRaycastHit.point.x - boxCollider.bounds.min.x; }
    ...
    bool hitBackface(Vector3 rayOrigin, Vector3 rayDirection, RaycastHit2D raycastHit)
    { return raycastHit && raycastHit.collider.OverlapPoint(rayOrigin); }
    
    bool hitFromBottom(Vector3 rayOrigin, Vector3 rayDirection, RaycastHit2D raycastHit)
    { return raycastHit && raycastHit.normal.y < 0.0f; }

    bool hitOneWayPlatform(RaycastHit2D raycastHit)
    { return raycastHit && (1 << raycastHit.collider.gameObject.layer & oneWayPlatformMask) != 0; }
    ...
    void moveVertically( ref Vector3 deltaMovement )
    {
        ...
        for( var i = 0; i < totalVerticalRays; i++ )
        {
            ...
            _unstuckRaycastHit = Physics2D.Raycast( ray, -rayDirection, rayDistance, mask );

            if (_unstuckRaycastHit && !hitOneWayPlatform(_unstuckRaycastHit))
            {
                if (isGoingUp)
                { deltaMovement.y = _unstuckRaycastHit.point.y - boxCollider.bounds.max.y; }
                else
                { deltaMovement.y = _unstuckRaycastHit.point.y - boxCollider.bounds.min.y; }
            ...
            if (_raycastHit)
            {
                ...
                if (_raycastHit && hitOneWayPlatform(_raycastHit))
                {
                    hitValid = !hitBackface(ray, rayDirection, _raycastHit) && 
                               !hitFromBottom(ray, rayDirection, _raycastHit);
                }
                else
                { hitValid = true; }
            }
    ...
    private void handleVerticalSlope( ref Vector3 deltaMovement )
    {
        ...
        _unstuckRaycastHit = Physics2D.Raycast( slopeRay, -rayDirection, slopeCheckRayDistance, platformMask );
